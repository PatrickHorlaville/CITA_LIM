...
def pltdata(pixbm, pntanx, pntany):
...
#    pntany  = 0.0
    ...
    VXL  = vx[(x > (bmctrx-slope)*z) & (x< (bmctrx+slope)*z) & (y>(bmctry-slope)*z) & (y< (bmctry+slope)*z) & (r < (max(z))) & (r > (min(z)*np.sqrt((max(z)**2+max(x)**2))/max(z)))]
    VYL  = vy[(x > (bmctrx-slope)*z) & (x< (bmctrx+slope)*z) & (y>(bmctry-slope)*z) & (y< (bmctry+slope)*z) & (r < (max(z))) & (r > (min(z)*np.sqrt((max(z)**2+max(x)**2))/max(z)))]
    VZL  = vz[(x > (bmctrx-slope)*z) & (x< (bmctrx+slope)*z) & (y>(bmctry-slope)*z) & (y< (bmctry+slope)*z) & (r < (max(z))) & (r > (min(z)*np.sqrt((max(z)**2+max(x)**2))/max(z)))]
    RSL = r_to_z(RL)
